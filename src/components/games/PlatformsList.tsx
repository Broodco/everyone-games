...
const getPlatformsIconsOrNames = (
  platformsList: PlatformInfo[]
): PlatformIconOrNameType[] => {
  ...
  platformsList.map((platform) => {
    ...
    if (
      platformIconSrc &&
      platformsIconsOrNamesList.filter(
        (elem) => elem.isIcon && elem.value === platformIconSrc
      ).length === 0
    ) {
      platformsIconsOrNamesList.push({
        isIcon: true,
